...
const LeftPane = ({ activeMenu, setActiveMenu, folders, onAddFolder, onDeleteFolder, onBulkImport, onShowStats, onShowShop, onShowMindPalace, onShowManual, onSyncConfig, onMovePaper, playSfx }: LeftPaneProps) => {
  ...
  return (
    <div className={`h-full border-r-4 flex flex-col w-72 shrink-0 relative overflow-hidden z-20 transition-colors duration-500 ${isVelvet ? 'bg-[#000033] border-[#D4AF37]' : 'bg-phantom-black border-phantom-red'}`}>
        ...
        <div className="h-full overflow-y-auto custom-scrollbar p-6 flex flex-col">
            <div className="mb-10 relative transform -rotate-6 origin-top-left cursor-default -ml-4 shrink-0">
            ...
            <nav className="space-y-4 z-10 mb-8 shrink-0">
            ...
            <div className={`flex items-center justify-between border-b-2 pb-2 mb-4 z-10 shrink-0 ${isVelvet ? 'border-[#D4AF37]' : 'border-zinc-700'}`}>
            ...
            <div className="space-y-2 z-10 pr-2">
                <button 
